...
export class PhotonProvider {
  ...
  public mapzenKey: string = "mapzen-5ZktmUZ";
  public coords: any;
  public focusLng: number = -122.4;
  public focusLat: number = 37.7;
  public source: string = "osm";
  ...
  searchRoute(start: any, end: any) {
    //1.
    console.log("Pulling coords from " + start + " and " + end);

    return new Promise(resolve => {
      // We're using Angular HTTP provider to request the data,
      // then on the response, it'll map the JSON data to a parsed JS object.
      // Next, we process the data and resolve the promise with the new data.
      this.http.get('https://search.mapzen.com/v1/autocomplete?text=' + start + '&sources=' + this.source + '&focus.point.lat=' + this.focusLat + '&focus.point.lon=' + this.focusLng + '&api_key=' + this.mapzenKey)
        .map(res => res.json())
        .subscribe(data => {
      ...
      this.http.get('https://search.mapzen.com/v1/autocomplete?text=' + end + '&sources=' + this.source + '&focus.point.lat=' + this.focusLat + '&focus.point.lon=' + this.focusLng + '&api_key=' + this.mapzenKey)
        .map(res => res.json())
        .subscribe(data => {
  ...
  autocomplete(query: any) {
    ...
    this.http.get('https://search.mapzen.com/v1/autocomplete?text=' + query + '&sources=' + this.source + '&focus.point.lat=' + this.focusLat + '&focus.point.lon=' + this.focusLng + '&api_key=' + this.mapzenKey)
      .map(res => res.json())
      .subscribe(data => {
        resolve(data);
